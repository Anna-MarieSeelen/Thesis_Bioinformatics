...
Description: takes result csv files of MS2LDA and MS2Query and output pdf that gives an overview of motifs and analogs.
It also rearranges the data so it can be used to build a two-mode network.
...
*path_file_with_Motif_fragments_csv*
    ...
    path_to_file_with_MS2Query_csv:
    path_file_with_MS2LDA_csv:
    path_file_with_Motif_fragments_csv:
"""
...
def connection_Mass2Motifs_to_documents(path_file_with_MS2LDA_csv: str) -> None:
    ...
    # We only want the motifs found in more than 5 spectra in the dataframe
    df_motif_and_list = df_motif_and_list[df_motif_and_list["Document+Probability+Overlap"].str.len() >= 5]
    ...
    # Again we only want the motifs found in more than 5 spectra in the dataframe
    df_motif_and_doclist = df_motif_and_doclist[df_motif_and_doclist['Document'].str.len() >= 5]
...
def information_document_node(path_to_file_with_MS2Query_csv: str) -> None:
    """
    ...
    df_analog_above_threshold = df_MS2Query[df_MS2Query.ms2query_model_prediction > 0.7]
...
def information_document_Mass2Motif_node(path_file_with_MS2LDA_csv_fragments: str) -> None:
    """
    ...
    features
    :return: pandas dataframe with the motifs as index and each feature and probability in a list in seperate column.
